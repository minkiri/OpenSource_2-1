...
from sensor_msgs.msg import Joy ,PointCloud ##for Joystick feedback 230606 YJ
...
def draw_lane_img(img, leftx, lefty, rightx, righty):
    ...
    for ctr in zip(leftx, lefty):
        point_np = cv2.circle(point_np, ctr, 3, (255,0,0),-1)

    #Right Lane
    for ctr in zip(rightx, righty):
        point_np = cv2.circle(point_np, ctr, 3, (0,0,255),-1)
...
class purePursuit :
    def __init__(
        ...
        self.platoon_sub = rospy.Subscriber('/platoon_go',Bool,self.platoon_callback)
        self.front_dis_sub = rospy.Subscriber('front_distance',Float32,self.front_dist_callback)
        self.center_point_sub = rospy.Subscriber('center_points',PointCloud,self.center_point_callback)
        #####Publisher#####
        self.cmd_pub = rospy.Publisher('/ctrl_cmd', CtrlCmd, queue_size=1)
        self.motor_pub = rospy.Publisher('/high_level/ackermann_cmd_mux/input/nav_0', AckermannDriveStamped, queue_size=1)
        self.current_speed_pub = rospy.Publisher('/front_current_speed',Float32,queue_size=1)
        
        self.lpath = None
        self.ctrl_msg = CtrlCmd()
        self.current_vel = None
        self.motor_msg = AckermannDriveStamped()
        self.current_speed_msg = Float32()
        self.joy_msg = Joy()
        self.dist_msg = Float32()
        self.platoon_msg = Bool()
        self.obs_msg = PointCloud()
    ...
    def platoon_callback(self,msg):
        platoon_msg = msg.data
        if platoon_msg == True :
            self.platoon_go = True
        else :
            self.platoon_go = False
    ...
    def steering_angle(self):
        ...
        if self.is_look_forward_point :
            ...
            steering_deg= 1.1*atan2(2*self.vehicle_length*sin(theta),self.lfd)
            #print(self.lfd)
            self.ctrl_msg.steering=steering_deg#*math.pi/180
            self.motor_msg.drive.steering_angle= steering_deg*3.5           
            self.current_speed_msg = self.motor_msg.drive.speed

            if len(self.obs_msg.points)>0 : ##for AEB 230611 YJ
                if(self.dist_msg.data<0.6):
                    self.stop = True
            else:
                self.stop = False     
            
            # if self.stop:
            #     self.motor_msg.drive.speed = 0.0
            #     print("E-STOP")
            if self.platoon_go == False:
                self.motor_msg.drive.speed = 0.0
                print("STOP")
            elif self.platoon_go == True:
                self.motor_msg.drive.speed = 0.3
                print("GO")
            
            print(self.motor_msg.drive.speed)

        else : 
            self.ctrl_msg.steering=0.0
            self.motor_msg.angle = 0.0
    ...
    def pub_cmd(self):
        ...
        self.current_speed_pub.publish(self.current_speed_msg)
